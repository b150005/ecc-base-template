...
t_no_scope_bleed() {
  ...
  # the relocation legitimately switches detector parse/scan targets
  # from .claude/CLAUDE.md to .claude/ROADMAP.md. The scope-bleed guard
  # continues to apply to the remaining guarded files (ci-base.yml,
  # check-skill-invariants.sh, check-bilingual-parity.sh,
  # test-check-dangling-refs.sh, test-check-bilingual-parity.sh — all
  # unchanged by #22).
  local exempt_post_22=(
    ".claude/meta/scripts/check-dangling-refs.sh"
    ".claude/meta/scripts/check-roadmap-drift.sh"
    ".claude/meta/scripts/test-check-roadmap-drift.sh"
  )
