...
3. The 19 pre-seeded placeholder files were removed. `.claude/learn/knowledge/` is now empty at install; files are created on first teaching moment per domain (lazy-materialization).
...
|---|---|
| `.claude/growth/` | `learn/` |
| `.claude/growth/config.json` | `.claude/learn/config.json` |
| `.claude/growth/preamble.md` | `.claude/skills/learn/preamble.md` |
| `.claude/growth/notes/<domain>.md` | `.claude/learn/knowledge/<domain>.md` |
| `.claude/skills/growth/SKILL.md` | `.claude/skills/learn/SKILL.md` |
| `docs/en/growth/` | `docs/en/learn/` |
| `docs/en/growth/domain-taxonomy.md` | `.claude/meta/references/domain-taxonomy.md` |
| `docs/ja/growth/` | `docs/ja/learn/` |
| `docs/ja/growth/domain-taxonomy.md` | `docs/ja/learn/domain-taxonomy.md` |
| `docs/en/growth-mode-explained.md` | `.claude/meta/references/learning-mode-explained.md` |
| `docs/ja/growth-mode-explained.md` → | `docs/ja/learning-mode-explained.md` |
| `docs/en/prd/developer-growth-mode.md` | `.claude/meta/prd/developer-learning-mode.md` |
| `docs/ja/prd/developer-growth-mode.md` | `docs/ja/prd/developer-learning-mode.md` |
| `scripts/check-growth-invariants.sh` | `.claude/meta/scripts/check-learn-invariants.sh` |

New paths added in v2.0.0 with no v1.x equivalent:

| New path | Purpose |
|---|---|
| `.claude/meta/references/migration/v1-to-v2.md` | This file |
| `docs/ja/migration/v1-to-v2.md` | Japanese translation of this file |
| `.claude/meta/references/examples/<domain>.md` | 19 Meridian-grounded worked examples (read-only reference) |
| `.claude/meta/adr/003-learning-mode-relocate-and-rename.md` | ADR recording the v2.0.0 decisions |
...
2. Confirm that `.gitignore` lists `.claude/learn/knowledge/` and `.claude/learn/config.json`. The `.claude/learn/knowledge/` directory does **not** exist on disk yet — per the lazy-materialize invariant in [ADR-003 §4](../adr/003-learning-mode-relocate-and-rename.md), it is created on the first teaching moment after `/learn on`.
...
If `.claude/learn/config.json` (or `.claude/growth/config.json` in v1.x) exists and you have knowledge files you want to preserve, follow these steps.
...
Each domain file moves from `.claude/growth/notes/` to `.claude/learn/knowledge/`. Run one command to move all of them at once:
...
If `.claude/growth/preamble.md` was customized in your fork, review the new `.claude/skills/learn/preamble.md` that ships with v2.0.0 before discarding the old file. The v2.0.0 preamble reflects the new paths and terminology; your customizations may need to be reapplied.

### Step 4: Update .gitignore

Replace any existing entry that ignores `.claude/growth/notes/` with one that ignores `.claude/learn/knowledge/`:
...
| `.claude/growth/notes/` | `.claude/learn/knowledge/` |
| `.claude/growth/` | `learn/` |
...
```bash
bash .claude/meta/scripts/check-learn-invariants.sh
```
