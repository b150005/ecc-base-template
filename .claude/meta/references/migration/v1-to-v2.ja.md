> このドキュメントは `.claude/meta/references/migration/v1-to-v2.md` の日本語訳です。英語版が原文（Source of Truth）です。
...
3. 事前シードされていた 19 個のプレースホルダーファイルが廃止されました。`.claude/learn/knowledge/` はインストール時には空であり、ファイルはドメインごとに最初の教示機会が発生したときに作成されます（遅延マテリアライゼーション）。
...
| 新しいパス | 用途 |
|---|---|
| `.claude/meta/references/migration/v1-to-v2.md` | 英語原文（このドキュメント） |
| `docs/ja/migration/v1-to-v2.md` | 本ファイル（日本語訳） |
| `.claude/meta/references/examples/<domain>.md` | 19 件の Meridian ベース正典ドメイン例（読み取り専用参照） |
| `.claude/meta/adr/003-learning-mode-relocate-and-rename.md` | v2.0.0 の決定を記録した ADR |
...
2. `.gitignore` に `.claude/learn/knowledge/` と `.claude/learn/config.json` が記載されていることを確認してください。`.claude/learn/knowledge/` ディレクトリ自体はまだディスク上に存在しません — [ADR-003 §4](../adr/003-learning-mode-relocate-and-rename.md) の lazy-materialize 不変式により、`/learn on` 後の最初の教育モーメントで初めて作成されます。
3. 以上で完了です。機能はデフォルトで OFF のままです。

## Learning Mode を有効化してナレッジを蓄積しているフォーク

`.claude/learn/config.json`（v1.x では `.claude/growth/config.json`）が存在し、保持したいナレッジファイルがある場合は、以下の手順に従ってください。
...
各ドメインファイルを `.claude/growth/notes/` から `.claude/learn/knowledge/` に移動します。次のコマンドで一括して移動できます。
...
フォーク内で `.claude/growth/preamble.md` をカスタマイズしていた場合は、古いファイルを破棄する前に v2.0.0 の `.claude/skills/learn/preamble.md` を確認してください。v2.0.0 のプリアンブルは新しいパスと用語を反映しています。カスタマイズ内容は再適用が必要な場合があります。

### ステップ 4: .gitignore を更新する

`.claude/growth/notes/` を無視していた既存のエントリを `.claude/learn/knowledge/` を無視するエントリに置き換えてください。
