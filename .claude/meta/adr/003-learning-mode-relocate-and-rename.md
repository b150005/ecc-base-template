...
> **Implementation deferred to v2.2.0.** The decision to ship worked examples as read-only references at `docs/en/learn/examples/<domain>.md` is recorded here as part of ADR-003's design. The 19 Meridian-grounded files were originally planned for the v2.1.0 PR alongside the coaching pillar (ADR-004); for review-load reasons the v2.1.0 PR was split, with the coaching pillar shipping in v2.1.0 and the worked examples following in v2.2.0. No behavior depends on their presence — the lazy-materialize invariant (§4) stands alone.

To give learners an immediate sense of what a populated knowledge file looks like, v2.2.0 ships 19 worked examples at `docs/en/learn/examples/<domain>.md` (and Japanese mirrors at `docs/ja/learn/examples/`). (The original plan in this ADR scheduled them for v2.1.0; the v2.1.0 PR was split for review-load reasons, with the coaching pillar shipping in v2.1.0 and the worked examples following in v2.2.0.) These are:
...
The examples are coexistence-friendly with lazy-materialize: after v2.2.0 ships, a fresh fork has an empty `learn/knowledge/` and 19 populated `docs/en/learn/examples/` files. Learners read the examples to understand the format, then grow their own knowledge base as teaching moments fire. Before v2.2.0 ships (i.e., on v2.0.0 and v2.1.0), the `docs/en/learn/examples/` tree does not exist; learners rely on the preamble and explainer alone.
...
11. **Filesystem migration.** Delete `.claude/growth/notes/` (all 19 placeholders). Relocate `learn/config.json` and `learn/preamble.md` from their old locations. Creation of `docs/en/learn/examples/` with 19 Meridian-grounded example files is **deferred to v2.2.0** — see the "Worked examples deliverable" subsection below.
...
### Worked examples deliverable (deferred to v2.2.0)

> **Status:** Decision recorded here; implementation deferred to v2.2.0. Originally planned for the v2.1.0 PR alongside the coaching pillar, the v2.1.0 PR was split for review-load reasons (coaching pillar in v2.1.0; worked examples in v2.2.0). The files listed below do not exist in the v2.0.0 or v2.1.0 trees and are authored in the v2.2.0 release.
