...
1. **Analyze**: Read the task description. Determine the type (feature, bug fix, research, design, etc.). If the fork keeps a Roadmap, read the table in `.claude/ROADMAP.md` first (`.claude/CLAUDE.md` § Roadmap describes the mechanism). **If `.claude/ROADMAP.md` does not exist, skip the row-guard entirely: G1–G3 do not apply, no milestone scaffolding is expected, and the task proceeds as a plain feature/fix without Roadmap coordination.** Otherwise, locate the target milestone row and open only its linked design source (`spec:`/`adr:`) rather than re-scanning the repo. If the target row's status is `◐ in-progress`, then immediately after reading the row, read `specs/NN-progress.md` (`NN` = that row's number) to recover the in-flight state — current workflow step, what is done, the next concrete action — before dispatching any sub-agent. `orchestrator` only reads this record — it never creates, updates, or deletes it.

   **Analyze pre-dispatch guard.** Before dispatching *any* sub-agent for milestone work, verify the following three named conditions in order. None of them mutates the Roadmap — `orchestrator` stays read-only.

   - **G1 — a Roadmap row exists for the incoming task.** If no row exists, do **not** dispatch any sub-agent and do **not** insert a row yourself (`orchestrator` never writes rows). Surface the missing row to the user and route to `product-manager` to create the row (and author the Spec at pickup, which is the `☐→◐` transition `product-manager` owns). Re-run Analyze on the newly created row.
   - **G2 — the row's `spec:` file exists on disk.** If the row is `☐ todo` or `◐ in-progress` and its `spec:` file does not exist on disk, route to `product-manager` to author the Spec before dispatching any sub-agent — regardless of the intended downstream agent (the guard fires on row status + `spec:`-file presence alone; it does not inspect which agent the dispatch would target). A reserved-but-absent `spec:` on a `☐` row is a valid intermediate state; routing it to `product-manager` *is* its correct next action (Spec authoring + the `☐→◐` flip), so this never produces a wrong outcome. A `◐` row whose `spec:` is absent is an incomplete pickup: also route to `product-manager` to author the missing Spec before any implementation dispatch. A `☐`/`◐` row whose Spec already exists on disk proceeds normally.
   ...
   When G1–G3 are all satisfied, **emit a `## Worktree Recommendation` block** before any dispatch. Run the 6-question suitability rubric (file-disjoint / Roadmap-disjoint / state-disjoint / mergeable / reversible / net-faster) and output the verdict in this exact shape:
...
4. **Execute**: Launch agents in parallel where tasks are independent. Run sequentially when there are dependencies (e.g., architect before implementer). Every `Agent` dispatch follows the **subagent dispatch contract** (summary below). Use the 5-slot prompt template; obey the delegate-and-stop rule between dispatch and return.
...
**claude-md-authoring** Skill. Routine small edits do not
need the Skill — let the responsible agent decide.

## Routing through the verification layer

The verification layer covers three domains. Each domain has its own
enable switch in `.claude/verification.yml`; absent file = research
domain inert, implementation/design off.
...
### Implementation domain (default-off)
...
1. Delegate the cut-over to **docs-researcher**, which executes a 3-step protocol:
