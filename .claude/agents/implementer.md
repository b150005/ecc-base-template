...
## Upstream workaround marker

When the implementation is a workaround for an upstream defect, place a marker comment at every workaround site so the next person who reads the code understands why it exists:

```
WORKAROUND-UPSTREAM(<owner>/<repo>#<issue>, fixed=>=<version>)
```

The `<owner>` and `<repo>` come from the upstream issue URL; the version is the expected upstream fix. Remove the marker (and the workaround code) once the upstream fix lands and the project's dependency is bumped past `<version>`.
