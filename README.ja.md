...
Growth Mode が有効なとき、チーム内の各エージェントは、応答を確定する前に `.claude/growth/config.json` を読み込みます。`enabled` が `true` であり、かつそのエージェントに対するスコープ設定も `true` であれば、エージェントは主たる出力のあとに 2 つのトレーラー節を追加します。生成される成果物そのものは変更されません。本番コードに教育目的のインラインコメントが足されることもありません。`docs/` や `.claude/growth/` の外には、いかなるファイルも書き込まれません。トレーラー節はチャット応答のなかに現れ、各エージェントは `.claude/growth/preamble.md` に定められた非破壊的な拡充操作に従って、対応するドメインファイル（`.claude/growth/notes/<domain>.md`）も併せて更新します。知識ベースは時系列のログではなく、ドメイン別に整理されています。セッションを重ねるたびに、実プロジェクトでの判断の蓄積に応じて、既存の節が深まっていくか、新しい節が追加されていきます。
...
**設定ファイル** は状態の入口です。`.claude/growth/config.json` は、セッションをまたいでレベルを保持します。最初の `/growth on` 呼び出しで作成されます。そのスキーマは次のとおりです。
...
チームの一部からだけノートを受け取りたい場合には、個々のエージェントのスコープを `false` に設定できます。

**CLAUDE.md のポインタ** は発見性のための入口です。`.claude/CLAUDE.md` の 1 行で、機能の存在と有効化方法を伝えます。Growth Mode を利用するために CLAUDE.md を編集する必要はありません。呼び出し方を覚えていなくても、ここから辿れるようにしてあるだけです。
...
│   ├── growth/                            # Growth Mode の実行時ファイル一式（既定で gitignore）
│   │   ├── config.json                    # enabled フラグ、レベル、エージェントごとのスコープ
│   │   ├── preamble.md                    # 全エージェントが共有する拡充コントラクト
│   │   ├── notes/                         # 19 のドメイン別ノートファイル（教科書本体）
│   │   │   ├── architecture.md
│   │   │   ├── api-design.md
│   │   │   ├── data-modeling.md
...
補足: `.claude/growth/` ディレクトリとその中身は、`/growth` Skill とエージェントによって実行時に作成されます。最初の使用までは、リポジトリのルートには存在しません。`config.json` と `notes/` は既定で gitignore されています。共有へオプトインする手順については、上の「ノートはデフォルトで非公開」の節を参照してください。
