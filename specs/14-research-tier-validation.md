...
- Specifying the exact form of the validation mechanism (a new agent-prompt constraint, a new detector, an extension of an existing artifact, a protocol amendment, or a combination) or whether it warrants a new ADR-021 or an amendment. That is the architect's structural decision (see Risk R-01). <!-- ref-allow: ADR-021 is the next unused ADR number cited as a possible outcome of the architect's structural decision for #14; it does not yet exist by design -->
...
7. **Structural HOW deferred to architect.** Whether #14 ships as (a) an amendment to `research/protocol.md` that elevates the T1 scope line to a mandatory pre-classification rule with a machine-readable marker, (b) a new agent-prompt constraint in `docs-researcher.md` that fires before a Tier declaration is accepted, (c) a new standalone contract document or SKILL.md section that names the auth→T1 requirement as an independently-checkable obligation, (d) a detector-style script that audits research outputs in the session or in CI for T2/T3 declarations on auth-keyword-matching topics, or (e) some combination — all deferred to the architect's forthcoming decision. The architect applies the ADR-018 Alternative-B discriminator verbatim. ADR-021 is the next unused number (ADRs 016–020 are consumed). The architect decides; this Spec does not. <!-- ref-allow: ADR-021 is the next unused ADR number cited as a possible outcome of the architect's structural decision for #14; it does not yet exist by design -->
...
- Authoring ADR-021 — that is the architect's responsibility if the structural decision warrants a new ADR. <!-- ref-allow: ADR-021 is the next unused ADR number cited as a possible outcome of the architect's structural decision for #14; it does not yet exist by design -->
...
- ADR-018 (bilingual parity detector) — the source of the Alternative-B discriminator the architect must apply when deciding whether #14 warrants ADR-021 or an amendment; the most recently consumed ADR number before ADR-019/020 is 018 (confirming 021 is the next unused number after the 019/020 sequence) <!-- ref-allow: ADR-021 is the next unused ADR number cited as a possible outcome of the architect's structural decision for #14; it does not yet exist by design -->
- ADR-020 (ECC-absent degraded-review signal) — the most recently consumed ADR number; ADR-021 is the next unused number if the architect's decision warrants a new ADR <!-- ref-allow: ADR-021 is the next unused ADR number cited as a possible outcome of the architect's structural decision for #14; it does not yet exist by design -->
