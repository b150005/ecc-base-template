...
**Note (ADR-018 amendment — 2026-05-17):** AC#5's and AC#7's phrase "in-scope tree" is authoritatively refined by the `## Amendment — 2026-05-17 (in-scope granularity — per-pair, not per-directory)` section of ADR-018 (`.claude/meta/adr/018-bilingual-parity-detector.md`) to per-FILE-PAIR granularity. Under that refinement, a lone `<stem>.md` with no `<stem>.ja.md` sibling is the sanctioned EN-only complement and does NOT fail presence-parity; presence-parity fails only on an orphaned `<stem>.ja.md` with no `<stem>.md` counterpart (AC#6). This is why the template is green-by-construction (AC#9) despite EN-only `.md` files existing in convention-bearing trees: those files are sanctioned EN-only complements, not parity failures. AC#7's explicit deferral to ADR-018 scopes AC#5; the amendment is the authoritative refinement layered on top of the historical contract.
