...
The knowledge directory at `learn/knowledge/` is a living reference organized by domain, not a chronological log. Agents enrich, deepen, and refine the same domain files over time as relevant decisions accumulate across sessions. A session does not append a dated entry; it opens the file for the relevant domain and either adds a new section, deepens an existing section, or refines an older entry as understanding matures.

No domain files are pre-seeded. `ls learn/knowledge/` returns nothing on a fresh clone. Files appear only when a real teaching moment earns them. After many sessions on a real project, a subset of the 19 canonical domains will have grown substantially. A developer's directory might look like this:
...
**What gets written to `learn/knowledge/architecture.md`:**
