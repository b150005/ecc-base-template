> このドキュメントは `docs/en/learn/examples/performance-intuition.md` の日本語訳です。英語版が原文（Source of Truth）です。
