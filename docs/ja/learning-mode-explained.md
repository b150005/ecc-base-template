> このドキュメントは `docs/en/learning-mode-explained.md` の日本語訳です。英語版が原文（Source of Truth）です。
...
2 つの節は `## Learning: taught this session` と `## Learning: knowledge diff` です。前者は、その応答で何を伝えたか（判断の根拠、トレードオフ、検討した代替案とそれが選ばれなかった理由、その推論がプロジェクトの ADR や信頼できる外部ドキュメントのどこに裏づけられているか）を記録します。後者は、知識ベースに対して行った保守操作を記録します。`learn/knowledge/` 配下のどのドメインファイルに手を入れたか、そのファイルのどの節か、どの種類の操作を適用したか（追加／深化／洗練／修正／新規ドメイン）を示します。ノートは「引用」するのであって、「説教」はしません。
...
数多くのセッションを経た実プロジェクト上で十分に蓄積されたナレッジファイルがどのような姿になるかを把握するには、`docs/ja/learn/examples/<domain>.md` 配下のワークサンプルを参照してください。これらは共通の架空プロジェクト (Meridian) を題材とした読み取り専用のリファレンスで、エージェントは読んだり書き込んだりしません。自分の `learn/knowledge/` ディレクトリに実コンテンツが蓄積される前に、期待値を調整するために存在します。設計の根拠は [ADR-003 §5](adr/003-learning-mode-relocate-and-rename.md) を参照してください。

---

## Learning Mode が ON のときに何が変わるか

Learning Mode が有効なとき、チーム内の各エージェントは応答を確定する前に `learn/config.json` を読み込みます。`enabled` が `true` であれば、エージェントは主たる出力のあとに 2 つのトレーラー節を追加します。生成される成果物そのものは変更されません。本番コードに教育目的のインラインコメントが足されることもありません。`docs/` や `learn/` の外にはいかなるファイルも書き込まれません。トレーラー節はチャット応答のなかに現れ、各エージェントは `learn/preamble.md` に定められた非破壊的な拡充操作に従って、対応するドメインファイル（`learn/knowledge/<domain>.md`）も併せて更新します。知識ベースは時系列のログではなく、ドメイン別に整理されています。セッションを重ねるたびに、実プロジェクトでの判断の蓄積に応じて、既存の節が深まっていくか、新しい節が追加されていきます。
...
`learn/knowledge/` 配下のナレッジディレクトリは、時系列のログではなく、ドメイン別に整理された「生きたリファレンス」です。エージェントは、セッションをまたいで関連する判断が積み重なるにつれて、同じドメインファイルを拡充し、深め、洗練させていきます。各セッションでは、日付付きのエントリを追記するのではなく、関連するドメインのファイルを開き、新しい節を追加するか、既存の節を深めるか、理解が成熟するにつれて古いエントリを洗練していきます。

ナレッジディレクトリは遅延マテリアライズされます。フレッシュなクローンの時点では `learn/knowledge/` の下にファイルは存在せず、あるドメインで初めて学びの機会が発生したときに初めてそのドメインファイルが作成されます。多くのセッションを重ねたあとの開発者側のディレクトリは、たとえば次のような姿になり得ます。
...
**`learn/knowledge/architecture.md` に書き込まれる内容:**
...
Learning Note は、生成された成果物のなかには決して現れません。本番ファイルのインラインコメントとしても、テストコードの一部としても、ドキュメントの埋め込みとしても出現しません。Learning Note はあくまで、エージェントのチャット応答の末尾の節です。Learning Mode が ON のときに implementer が書くコードは、OFF のときに書くコードとまったく同じです。これはエージェント出力のハッシュ照合で担保しているのではなく（LLM の出力は非決定的であり、ゴールデンファイルに対する回帰テストは不安定テストに劣化するため）、`scripts/check-learn-invariants.sh` の 3 つの決定論的な CI チェック（`learn` Skill の `disable-model-invocation: true`、すべての Learning 対応エージェントプロンプトのガード分岐、gitignore の方針）によって強制しています。
...
- `learn/preamble.md` — 各 Learning 対応エージェントが実行時に従う拡充コントラクト。コーチングピラーをカバーする §§15–20 を含みます。
