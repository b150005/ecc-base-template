...
# Learning Mode default-off invariant. Runs in CI on every PR.
#
...
# ("Enforcement: default-off invariant") and docs/en/adr/003-learning-mode-relocate-and-rename.md
...
#      also contains the guard-branch marker referencing learn/config.json
#   3. .gitignore ignores learn/knowledge/ and learn/config.json,
...
# Updated in ADR-003 (2026-04-24) from `## Growth Domains` to `## Learning Domains`
# and guard-branch marker updated from `.claude/growth/config.json` to `learn/config.json`.
...
guard_marker="learn/config.json"
...
if [[ ! -f "$gitignore" ]]; then
  ...
  check_ignore_line "learn/knowledge/"
  check_ignore_line "learn/config.json"
fi

if [[ ! -f "$example" ]]; then
  fail_check "$example not found"
elif grep -Fq "!learn/knowledge/" "$example"; then
